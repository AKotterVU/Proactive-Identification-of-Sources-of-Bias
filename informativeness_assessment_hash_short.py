# informativeness_assessment_hash.py
"""Given a model type, data, disparities data from a previous model 
iteration, and a list of SDoH features, determines which if any of the 
disparities are significantly reduced when exactly one SDoH feature is 
included as a model input feature. Iterates separately over each of 
the listed SDoH features.
1-13-25: Updated to use CSVs de-identified with hashes instead of CSNs 
for use on non-PHI servers. Also accepts one SDoH as a command-line 
argument instead of iterating through all SDoHs. Saves disparities and 
performances to CSV files with a randomized ID matching the TXT output.
Also 1-13-25: Does not perform statistical analysis. Just outputs the 
disparities and performance CSVs so that statistical analysis can be 
performed later with combined files from multiple runs.
...
from functions import sdoh_disparities
from functions import disparity_comparison
...
if __name__ == "__main__":
    ...
    old_disp_file = "disparities_RF_2025_1_10_joined.csv"
    ...
    outfile = f"informativeness_assessment_{sdoh}_{name}_{date_str}_{rn}.txt"
    
    # Get the new disparities
    new_disp_df, new_perf_df = sdoh_disparities(name, X, y, 
                                                dem_cols=["GENDER",
                                                          "SVI_QUARTILE", 
                                                          "ETHNICITY", 
                                                          "LANGUAGE", "RACE"], 
    ...
    new_disp_df.to_csv(f"disparities_{name}_add_{sdoh}_{date_str}_{rn}.csv", 
                       index=False)
    new_perf_df.to_csv(f"performance_{name}_add_{sdoh}_{date_str}_{rn}.csv", 
                       index=False)